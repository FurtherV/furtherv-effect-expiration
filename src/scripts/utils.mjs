 ...
 */
export function expireEffect(effect, reason) {
  const target = effect.target;
  const msg = `Effect '${effect.name}' targeting '${target.name}' is expired. Reason: '${reason}'`;

  console.debug(msg);
  ChatMessage.implementation.create({ content: msg });

  // TODO: Implement actual expiration logic (e.g., remove effect, trigger hooks)
}
